...
// CCV Requirements
double complex **signal_FFT, **signal_IFFT, **signal_CONJ, *signal_CONJPROD, *signal_ABSCONJPROD, *signal_CCVPHATFFT;
...
void pointDivision(double complex *dividend_s, double complex *divisor_s, double complex *return_data) {

    int i;

    for (i = 0; i < sub_buffer_size; ++i) {
		return_data[i] = (creal(dividend_s[i]) / creal(divisor_s[i]) ) + ( (cimag(dividend_s[i])) * I);
	}
...
void getAbs(complex double *data, complex double *return_data) {
	int i;

    for (i = 0; i < sub_buffer_size; ++i) {
		return_data[i] = cabs(data[i]);
	}
...
int main (int argc, char *argv[]) {
    ...
    for (i = 0; i < ports_number; ++i) {
        ...
        signal_CCVPHATFFT = (double complex *) fftw_malloc(sizeof(double complex) * sub_buffer_size);
        signal_CONJPROD = (double complex *) fftw_malloc(sizeof(double complex) * sub_buffer_size);
        signal_ABSCONJPROD = (double complex *) fftw_malloc(sizeof(double complex) * sub_buffer_size);
